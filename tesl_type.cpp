...
namespace tesl {
  namespace detail {
    ...
    template<typename T>
    constexpr TypeInfo make_static_type_info(StrView name) {
      return {
        name,
        sizeof(T),
        alignof(T),
  ...
  TypeInfo typeInfoBool = detail::make_static_type_info<Bool>(TESL_STRVIEW("Bool"));
  ...
  TypeInfo typeInfoStr = detail::make_static_type_info<Str>(TESL_STRVIEW("Str"));
