...
namespace tesl {
  struct Variant {
    const TypeInfo * _type = type_info_of<Null>;
    ...
    TESL_ALWAYS_INLINE const TypeInfo * get_type() const {
      return _type;
    }
    ...
    template<typename T>
    constexpr Variant(T v) : _type(type_info_of<T>) {
      if (_type->size <= variant_storage_size) {
