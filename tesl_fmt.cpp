...
FMT_FUNC void assert_fail(const char* file, int line, const char* message,
  ...
  if (error_code) {
    fprintf(stderr, "%s:%d: assertion failed: %s: %s", file, line, message, std::strerror(error_code));
  } else {
...
inline void fwrite_all(const void* ptr, size_t count, FILE* stream) {
  ...
  if (written < count)
    FMT_THROW_SYSTEM_ERROR(errno, "cannot write to file");
}
 ...
 public:
  ...
  auto get() -> int {
    ...
    if (result == EOF && ferror(file_) != 0)
      FMT_THROW_SYSTEM_ERROR(errno, "getc failed");
    return result;
  }
  ...
  void unget(char c) {
    if (ungetc(c, file_) == EOF)
      FMT_THROW_SYSTEM_ERROR(errno, "ungetc failed");
  }
