...
#define TESL_BIND_BUITIN_BARE_MEMBER_VARIABLE(type, var, name, sig) \
  ::tesl::detail::bind_member_variable(type, ::tesl::FunctionBare{fn, ::tesl::get_builtin_signature(sig)})
*/
#define TESL_BIND_BUITIN_BARE_MEMBER_FUNCTION(type, fn, sig, ...) \
  ::tesl::detail::bind_member_function(type, TESL_BUILTIN_SIGNATURE(sig), {fn, {__VA_ARGS__}})
